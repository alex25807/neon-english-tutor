...
export default function App() {
  ...
  if (!state.profile) return (<><SpaceBg /><Onboarding onComplete={handleOnboarding} state={state} onPatch={(p: Partial<Settings>) => setState((s) => ({ ...s, settings: { ...s.settings, ...p } }))} /></>);
  ...
  return (
    <div className="min-h-screen bg-space relative">
      ...
      {!active ? (
        <>
          ...
          <main className="relative z-10">
            ...
            {tab === "chat" && <ChatPanel state={state} onAppend={(msgs: ChatMsg[]) => setState((s) => ({ ...s, chat: [...s.chat, ...msgs].slice(-120) }))} onToast={pushToast} />}
          ...
          <footer className="relative z-10 text-center pb-8 pt-4">
            <p className="text-white/25 font-bold text-[11px]">Neon English • квест-репетитор • прогресс сохраняется автоматически</p>
          </footer>
        </>
      ) : (
      ...
      <Modal open={resetConfirm} onClose={() => setResetConfirm(false)}>
        ...
        <p className="text-white/70 font-bold text-sm mt-2 leading-relaxed">Уровень, опыт, выученные слова, достижения и чат будут удалены навсегда. Это действие нельзя отменить!</p>
        <div className="flex gap-3 mt-6">
          <NeonButton tone="dark" className="flex-1" onClick={() => setResetConfirm(false)}>Оставить</NeonButton>
          <NeonButton tone="pink" className="flex-1" onClick={doReset}>Сбросить</NeonButton>
        </div>
      </Modal>

      <Modal open={exitConfirm} onClose={() => setExitConfirm(false)}>
        <div className="text-center">
          <div className="w-16 h-16 mx-auto rounded-full bg-gradient-to-br from-neon to-pinky flex items-center justify-center shadow-[0_0_30px_rgba(0,229,255,0.5)]">
          ...
          <p className="text-white/70 font-bold text-sm mt-3 leading-relaxed">
            {state.profile?.name}, ты молодец! Весь твой прогресс сохранён. Возвращайся скорее — новые квесты и слова ждут!
          </p>
          <div className="grid grid-cols-3 gap-3 mt-5">
            <div className="bg-panel border-2 border-edge rounded-xl p-3">
              <div className="font-display text-lg text-goldy">{state.xp}</div>
              <div className="text-[9px] font-extrabold uppercase tracking-widest text-white/40 mt-0.5">Опыт</div>
            </div>
            <div className="bg-panel border-2 border-edge rounded-xl p-3">
              <div className="font-display text-lg text-pinky">{Object.keys(state.cards).filter((id) => state.cards[id].seen > 0).length}</div>
              <div className="text-[9px] font-extrabold uppercase tracking-widest text-white/40 mt-0.5">Слов</div>
            </div>
            <div className="bg-panel border-2 border-edge rounded-xl p-3">
              <div className="font-display text-lg text-limey">{state.streak.count}</div>
              <div className="text-[9px] font-extrabold uppercase tracking-widest text-white/40 mt-0.5">Серия</div>
            </div>
          </div>
          <div className="flex gap-3 mt-6">
            <NeonButton tone="dark" className="flex-1" onClick={() => setExitConfirm(false)}>Остаться</NeonButton>
            <NeonButton tone="pink" className="flex-1" onClick={() => { setExitConfirm(false); window.close(); }}>Выйти</NeonButton>
          </div>
          <p className="text-white/30 font-bold text-[10px] mt-3">Если вкладка не закроется — просто закройте браузер</p>
        </div>
      </Modal>
    </div>
  );
