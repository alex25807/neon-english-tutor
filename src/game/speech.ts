...
if (ttsSupported) { loadVoices(); window.speechSynthesis.onvoiceschanged = loadVoices; }
...
function pickVoice(lang: string, gender: "male" | "female" | "default" = "default"): SpeechSynthesisVoice | null {
  ...
  const pool = googleVoices.length > 0 ? googleVoices : langVoices;

  if (gender === "default") return pool[0];

  const maleKeywords = ["male", "man", "david", "mark", "daniel", "james", "alex", "fred", "tom", "george", "rishi"];
...
export function speak(text: string, opts: { lang?: string; rate?: number; gender?: "male" | "female" | "default" } = {}): Promise<void> {
  return new Promise((resolve) => {
    ...
    try {
      ...
      u.volume = 1.0;
      activeUtterance = u;
      const done = () => { if (activeUtterance === u) activeUtterance = null; resolve(); };
...
export function startListening(opts: { lang: "en-US" | "ru-RU"; onFinal: (text: string) => void; onInterim?: (text: string) => void; onEnd: () => void; onError?: (err: string) => void }): { stop: () => void } | null {
  const Ctor = getSR();
  if (!Ctor) return null;
  let rec: SR;
  try { rec = new Ctor(); } catch { return null; }
  rec.lang = opts.lang; rec.continuous = false; rec.interimResults = true; rec.maxAlternatives = 3;
  rec.onresult = (e) => { let interim = ""; for (let i = e.resultIndex; i < e.results.length; i++) { const r = e.results[i]; if (r.isFinal) opts.onFinal(r[0].transcript); else interim += r[0].transcript; } if (interim && opts.onInterim) opts.onInterim(interim); };
...
export function normalizeSpoken(s: string): string { return s.toLowerCase().replace(/[^a-zа-яё\s]/gi, "").replace(/\s+/g, " ").trim(); }
...
export function speechMatches(target: string, heard: string): boolean {
  const t = normalizeSpoken(target); const h = normalizeSpoken(heard);
  if (!t || !h) return false;
  if (t === h) return true;
