...
export default function QuestPlayer({ zone, quest, state, onFinish, onExit, onToast }: { zone: Zone; quest: QuestDef | null; state: GameState; onFinish: (p: FinishPayload) => void; onExit: () => void; onToast: (t: Omit<Toast, "id">) => void }) {
  ...
  const run = useRef(buildQuestRun(zone, quest!, state)).current;

  if (!quest || run.length === 0) return (<div className="fixed inset-0 z-40 bg-void/95 backdrop-blur-sm overflow-y-auto py-8 px-4"><div className="relative z-10 max-w-md mx-auto text-center bg-deep/95 border-2 border-edge rounded-3xl p-8 anim-slide-up"><VoltRobot mood="cool" className="w-24 h-24 mx-auto anim-bobble" /><h2 className="font-display text-xl text-neon neon-cyan uppercase mt-4">Портал пуст</h2><p className="text-white/70 font-bold text-sm mt-2">Сейчас все слова повторены. Пройди новый квест!</p><NeonButton tone="cyan" size="lg" className="mt-6 w-full" onClick={onExit}>К карте галактики</NeonButton></div></div>);
...
function RunInner({ zone, quest, run, settings, playerName, onFinish, onExitRequest, onExitDirect, onRetry, onToast }: { zone: Zone; quest: QuestDef; run: Exercise[]; settings: Settings; playerName: string; onFinish: (p: FinishPayload) => void; onExitRequest: () => void; onExitDirect: () => void; onRetry: () => void; onToast: (t: Omit<Toast, "id">) => void }) {
