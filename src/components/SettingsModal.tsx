...
function Toggle({ on, onClick, label, desc }: { on: boolean; onClick: () => void; label: string; desc: string }) {
  return (
    <button onClick={onClick} className="w-full flex items-center gap-3 text-left group">
      ...
      <span>
        <span className="block font-extrabold text-sm text-white group-hover:text-neon transition-colors">{label}</span>
        <span className="block text-[11px] font-bold text-white/45">{desc}</span>
      </span>
    </button>
  );
}
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="flex items-center gap-3">
        <VoltRobot mood="cool" className="w-12 h-12" />
        <h3 className="font-display text-lg text-neon neon-cyan uppercase">Настройки</h3>
      </div>
      <div className="space-y-5 mt-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {(["default", "male", "female"] as const).map((g) => (
              <button key={g} onClick={() => onPatch({ voiceGender: g })} className={`btn-arcade px-3 py-2 rounded-xl border-2 font-extrabold text-xs transition-colors ${s.voiceGender === g ? "bg-neon text-[#04252b] border-[#8ffbff] shadow-[0_0_14px_rgba(0,229,255,0.5)]" : "bg-panel text-white/70 border-edge hover:border-neon/50 hover:text-white"}`}>
                {g === "default" ? "По умолчанию" : g === "male" ? "Мужской" : "Женский"}
              </button>
            ))}
          </div>
        ...
        <div>
          <div className="flex items-center justify-between">
            <span className="font-extrabold text-sm text-white">Скорость речи</span>
            <span className="font-display text-xs text-neon">{s.rate.toFixed(2)}x</span>
          </div>
        ...
        <div>
          <button onClick={() => speak("Hello! I am Volt. Let's learn English!", { rate: s.rate, gender: s.voiceGender })} disabled={!ttsSupported} className="btn-arcade w-full mt-2 inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl bg-panel2 border-2 border-edge text-neon text-xs font-extrabold hover:border-neon/60 disabled:opacity-30">
            <IconSpeaker className="w-4 h-4" /> Проверить голос
          </button>
        </div>

        <div className="rounded-xl border-2 border-edge bg-panel px-4 py-3 text-[11px] font-bold text-white/55 leading-relaxed">
          Весь прогресс — опыт, слова, серия дней, достижения и чат — сохраняется автоматически в этом браузере.
        </div>
        <div className="border-t-2 border-edge pt-4">
          <NeonButton tone="dark" className="w-full" onClick={onReset}>Сбросить весь прогресс</NeonButton>
        </div>
      </div>
    </Modal>
  );
