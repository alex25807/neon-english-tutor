...
import { IconCheck, IconFlame, IconLock, IconPlay, IconPortal, IconRocket, IconStar, IconTrophy, Planet, VoltRobot } from "./icons";
    ...
    <div className="px-4 max-w-5xl mx-auto w-full py-6">
      <div className="flex items-center gap-4 flex-wrap mb-6">
        <div className="flex items-center gap-3 bg-panel border-2 border-edge rounded-2xl px-4 py-3 flex-1 min-w-[260px]">
          ...
          <div className="min-w-0"><div className="font-display text-xs uppercase tracking-widest text-neon">Привет, {state.profile?.name}!</div><div className="text-white/75 font-bold text-sm leading-snug mt-0.5">Выбирай планету и вперёд — к новым словам и победам!</div></div>
        </div>
      ...
      {due > 0 && (
        <button onClick={onReview} className="btn-arcade w-full mb-6 rounded-2xl border-2 border-limey/60 bg-limey/10 px-5 py-4 flex items-center gap-4 text-left hover:bg-limey/15">
          ...
          <div className="flex-1"><div className="font-display text-limey uppercase tracking-wide text-sm">Портал повторения</div><div className="text-white/70 font-bold text-xs mt-0.5">Повтори {due} слов, которые пора освежить</div></div>
            ...
            <div key={zone.id} className={`rounded-2xl border-2 p-5 relative overflow-hidden ${unlocked ? "bg-deep/90 border-edge" : "bg-panel/40 border-edge/50 opacity-75"}`}>
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2"><h3 className="font-display text-lg uppercase tracking-wide truncate" style={{ color: zone.hue, textShadow: `0 0 10px ${zone.hue}55` }}>{zone.title}</h3>{!unlocked && <IconLock className="w-4 h-4 text-white/40 shrink-0" />}</div>
                ...
                {zone.quests.map((quest, qi) => {
                  ...
                  return (
                    <button key={quest.id} onClick={() => { if (!unlocked) { onLocked(`Нужен уровень ${zone.reqLevel}`); return; } if (!qUnlocked) { onLocked("Сначала пройди предыдущий квест!"); return; } onStart(zone, quest); }} className={`btn-arcade w-full rounded-xl border-2 px-4 py-3 flex items-center gap-3 text-left transition-colors ${!qUnlocked ? "border-edge bg-panel text-white/30" : isBoss ? "border-pinky/60 bg-pinky/10 text-white hover:bg-pinky/15" : "border-edge bg-panel text-white hover:border-neon/60 hover:bg-panel2"}`}>
                      ...
                      <div className="flex-1 min-w-0"><div className="font-extrabold text-sm truncate">{quest.title}</div><div className="text-[10px] font-bold text-white/45 uppercase tracking-wider">{isBoss ? "Битва с боссом" : qDone > 0 ? `Пройдено • ${stars}/3 ★` : `${quest.steps} заданий`}</div></div>
...
function StatCard({ icon, value, label }: { icon: React.ReactNode; value: number; label: string }) {
  return (<div className="bg-panel border-2 border-edge rounded-xl px-3 py-2.5 flex items-center gap-2">{icon}<div><div className="font-display text-lg text-white leading-none">{value}</div><div className="text-[9px] font-extrabold uppercase tracking-widest text-white/40 mt-0.5">{label}</div></div></div>);
}
